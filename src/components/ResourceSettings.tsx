    ...
    <section className="settings-pane resource-list-pane">
      ...
      <div className="resource-list">{summary?.items.map((item) => {
        ...
        return <article className="resource-list-item" key={item.id}>
          <div className="resource-list-cover">{item.coverPath ? <img src={`/${item.coverPath}`} alt="" /> : <PackageOpen />}</div>
          <div className="resource-list-copy"><strong>{item.title}</strong><small>{item.origin === 'custom' ? '用户添加 · 本地文件' : `${item.origin === 'builtin' ? '随应用内置' : `${formatBytes(item.size)} · ${Math.round(item.duration / 60)} 分钟`} · ${item.attribution.license} · ${item.attribution.creator}`}</small>{downloading && <div className="resource-inline-progress"><Progress value={currentProgress?.percent ?? 0} /><span>{currentProgress?.message}</span></div>}</div>
