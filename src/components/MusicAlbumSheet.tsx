...
import { Disc3, ImagePlus, Music2, Trash2 } from 'lucide-react'
...
export function MusicAlbumSheet({ album, tracks, onClose, onSave }: {
  album: MusicAlbum
  tracks: MusicTrack[]
  onClose: () => void
  ...
  return <Sheet open onOpenChange={(open) => { if (!open) onClose() }}>
    <SheetContent className="detail-drawer music-album-drawer" showCloseButton>
      ...
      <div className="album-sheet-body">
        ...
        <div className="album-track-list"><h3>收录歌曲</h3>{tracks.map((track, index) => <div className="album-track-item" key={track.id}><span>{index + 1}</span><div>{track.cover ? <img src={track.cover} alt="" /> : displayCover ? <img src={displayCover} alt="" /> : <Music2 size={15} />}</div><strong>{track.title}</strong><small>{formatDuration(track.duration)}</small></div>)}</div>
      </div>
