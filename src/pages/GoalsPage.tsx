  ...
  return <section className={`${embedded ? 'task-panel' : 'page'} goals-page goals-workspace`}>
    ...
    {goals.length > 0 && <div className="goal-dashboard">
      <button className="goal-focus-card" onClick={() => focusGoal && setSelectedId(focusGoal.id)} disabled={!focusGoal}>
        ...
        <div><small>现在最值得推进</small><strong className={focusGoal ? 'private-goal-title' : undefined}>{focusGoal?.title ?? '所有目标都已完成'}</strong><p className={focusGoal && (focusGoal.checkIns?.[0]?.nextStep || focusGoal.motivation) ? 'user-content' : undefined}>{focusGoal ? (focusGoal.checkIns?.[0]?.nextStep || focusGoal.motivation || '打开目标，为它写下一个清晰的下一步。') : '去记录一个新的方向吧。'}</p></div>
...
function GoalForm({ goals, initialParentId, onClose, onSubmit }: { goals: Goal[]; initialParentId: string | null; onClose: () => void; onSubmit: (value: GoalDraft) => void }) {
  ...
  return <Dialog open onOpenChange={(open) => { if (!open) onClose() }}><DialogContent className="modal-card goal-create-dialog"><DialogHeader className="modal-title"><div><span className="eyebrow">NEW GOAL</span><DialogTitle>{initialParentId ? '创建子目标' : '创建目标'}</DialogTitle><DialogDescription>先定义方向和成功标准，细节可以稍后补充。</DialogDescription></div></DialogHeader>
    ...
    <div className="goal-form-section"><strong>2 · 它与什么相关？</strong><label>上级目标<Select value={parentId} onValueChange={setParentId}><SelectTrigger><SelectValue className={parentId !== 'none' ? 'user-content' : undefined} /></SelectTrigger><SelectContent><SelectItem value="none">没有，作为顶层目标</SelectItem>{goals.filter((goal) => goal.status !== 'archived').map((goal) => <SelectItem key={goal.id} value={goal.id}><span className="user-content">{goal.title}</span></SelectItem>)}</SelectContent></Select></label><div className="form-row"><label>开始日期<DatePicker value={startDate} onChange={setStartDate} ariaLabel="目标开始日期" /></label><label>期望日期<DatePicker value={dueDate} min={startDate} onChange={setDueDate} ariaLabel="目标期望日期" invalid={!validDates} />{!validDates && <small className="form-error">不能早于开始日期</small>}</label></div></div>
...
function GoalDrawer({ goal, goals, onClose, onOpenGoal, onAddSubgoal, onSave, onCreateTodo, onDelete }: { goal: Goal; goals: Goal[]; onClose: () => void; onOpenGoal: (id: string) => void; onAddSubgoal: () => void; onSave: (patch: Partial<Goal>) => void; onCreateTodo: (title: string) => void; onDelete: () => void }) {
  ...
  return <><Sheet open onOpenChange={(open) => { if (!open) onClose() }}><SheetContent className="detail-drawer goal-drawer" showCloseButton><SheetTitle className="sr-only">{goal.title}</SheetTitle><SheetDescription className="sr-only">管理目标、成果、子目标和回顾记录</SheetDescription><div className="drawer-content">
    ...
    <Tabs defaultValue="overview" className="goal-detail-tabs"><TabsList className="goal-detail-tablist"><TabsTrigger value="overview">概览</TabsTrigger><TabsTrigger value="structure">成果与子目标</TabsTrigger><TabsTrigger value="reviews">回顾记录</TabsTrigger></TabsList>
      <TabsContent value="overview" className="goal-tab-panel"><label>为什么重要<Textarea value={goal.motivation ?? ''} onChange={(event) => onSave({ motivation: event.target.value })} placeholder="写给未来可能想放弃的自己" rows={3} /></label><label>上级目标<Select value={goal.parentId ?? 'none'} onValueChange={(value) => onSave({ parentId: value === 'none' ? null : value })}><SelectTrigger><SelectValue className={goal.parentId ? 'user-content' : undefined} /></SelectTrigger><SelectContent><SelectItem value="none">没有，作为顶层目标</SelectItem>{parentOptions.map((item) => <SelectItem key={item.id} value={item.id}><span className="user-content">{item.title}</span></SelectItem>)}</SelectContent></Select></label><div className="form-row drawer-dates"><label>开始日期<DatePicker value={goal.startDate} onChange={(value) => onSave({ startDate: value })} ariaLabel="目标开始日期" /></label><label>期望日期<DatePicker value={goal.dueDate} min={goal.startDate} onChange={(value) => onSave({ dueDate: value })} ariaLabel="目标期望日期" invalid={Boolean(goal.dueDate && goal.startDate && goal.dueDate < goal.startDate)} /></label></div><label>进度方式<Select value={goal.progressMode ?? 'manual'} onValueChange={(value) => onSave({ progressMode: value as GoalProgressMode })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="manual">手动更新</SelectItem><SelectItem value="metric">数值指标</SelectItem><SelectItem value="milestones">成果加权</SelectItem><SelectItem value="subgoals">子目标汇总</SelectItem></SelectContent></Select></label>
