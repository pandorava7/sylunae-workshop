...
export function WhiteNoisePage({ onNowPlayingChange }: { onNowPlayingChange: (item: ResourceItem | null) => void }) {
  ...
  return <section className="page white-noise-page">
    ...
    <div className={`white-noise-hero ${selected.origin === 'custom' ? 'private-media-background' : ''} ${playing ? 'is-playing' : ''}`} style={coverStyle(selected)}>
    ...
    <div className="white-noise-grid">{items.map((item) => {
      ...
      return <button type="button" key={item.id} className={`white-noise-card ${item.origin === 'custom' ? 'private-media-background' : ''} ${active ? 'active' : ''}`} style={coverStyle(item)} onClick={() => active && playing ? audioRef.current?.pause() : void playItem(item)} disabled={Boolean(busyId) && !downloading} aria-pressed={active}>
